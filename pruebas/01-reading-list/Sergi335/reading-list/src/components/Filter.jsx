...
export default function Filter ({ changeFilters }) {
  ...
  const handleChangeMinPages = (event) => {
    ...
    const label = document.querySelector('.labelPaginas')
    event.target.value > 0 ? label.classList.remove('hidden') : label.classList.add('hidden')
  }
  const handleChangeGenre = (event) => {
    ...
    const label = document.querySelector('.labelGenero')
    event.target.value !== 'all' ? label.classList.remove('hidden') : label.classList.add('hidden')
  }
  ...
  return (
        <div className="flex 2xl:gap-10 gap-2 my-0 border-slate-100 dark:border-[darkslategray] 2xl:border dark:2xl:border-0 py-[5px] 2xl:shadow-sm rounded px-[30px] dark:bg-[#100c18]">
          <div onClick={handleResetPages} className='labelPaginas dark:bg-[#100c18] w-max top-[109px] p-[15px] cursor-pointer hidden absolute 2xl:right-[30%] 2xl:top-[91px] bg-blue-500 2xl:py-[17px] 2xl:px-[35px] rounded-md text-white'><p>Número de páginas X</p></div>
                  <div onClick={handleResetGenre} className='labelGenero cursor-pointer w-max right-[32px] dark:bg-[#100c18] top-[109px] p-[15px] hidden absolute 2xl:right-[23%] 2xl:top-[91px] bg-blue-500 2xl:py-[17px] 2xl:px-[35px] rounded-md text-white'><p>Género X</p></div>

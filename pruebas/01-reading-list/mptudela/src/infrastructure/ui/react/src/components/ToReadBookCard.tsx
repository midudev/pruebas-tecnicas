...
export const ToReadBookCard = ({ book }: Props) => {
    const notify = () =>
        toast.success(`El libro "${book.title}" se ha eliminado de tu lista de lectura`, {
            position: toast.POSITION.BOTTOM_CENTER,
            theme: 'colored'
        });
    const handleClick = useGlobalContext(store => store.removeBookFromRead);
    return (
                ...
                <div className='hidden flex-1 p-6 md:block'>
                    <div className='flex flex-col h-full gap-6'>
                        ...
                        <div className='flex justify-end items-end h-full'>
                            ...
                            >
                                <BiBookmarkAltMinus className='h-6 w-6 fill-black ' />
                            </button>
                        </div>
                    </div>
                </div>
